...
class RiceProvider extends StateNotifier<List<Product>> {
  RiceProvider() : super([]); // Initializing the state as an empty list
  ...
  Future<void> loadCurryAndFryProducts() async {
    try {
      final response = await http.get(
        Uri.parse("$uri/api/rice-products"),
...
final riceProvider = StateNotifierProvider<RiceProvider, List<Product>>((ref) {
  return RiceProvider();
});
